...
def summary(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text(summary_text, parse_mode='Markdown')
    except Exception as e:
        update.message.reply_text(f"❌ Error getting summary: {str(e)}")
...
def search_command(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if not query:
            update.message.reply_text("Please provide a search query. Example: /search products")
            return
        
        df = bot.get_sheet_data()
        results = bot.search_data(query, df)
        update.message.reply_text(results, parse_mode='Markdown')
    except Exception as e:
        update.message.reply_text(f"❌ Error searching: {str(e)}")
...
def refresh(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text(f"✅ Data refreshed! Retrieved {len(df)} records from Google Sheets.")
    except Exception as e:
        update.message.reply_text(f"❌ Error refreshing data: {str(e)}")
...
def handle_message(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if df.empty:
            update.message.reply_text("❌ No data available. Please check your Google Sheets configuration.")
            return
        ...
        update.message.reply_text(f"❌ Sorry, I encountered an error: {str(e)}")
...
def main():
    ...
    try:
        ...
        # Create the Updater
        updater = Updater(token=bot.bot_token, use_context=True)
